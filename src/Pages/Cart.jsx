    ...
    <>
      <div className="w-full relative min-h-screen bg-black text-white">
        ...
        <div className="pt-25 px-5 lg:px-20 grid grid-cols-1 lg:grid-cols-3 gap-4 font-Satoshi">
          {Cards.map((item, i) => (
            <div key={i} className="bg-zinc-900 rounded-lg font-Satoshi flex flex-col gap-y-1 py-2">
              ...
              <button onClick={()=>dispatch(removeItems(item.id))} className='text-red-400 cursor-pointer'>Remove Product</button>
            </div>
          ))}
        </div>
        <button
